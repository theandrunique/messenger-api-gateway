...
export const handleChannelUpdateEvent = async (io: Server, data: any) => {
  const parseResult = ChannelUpdateEventSchema.safeParse(data);
  ...
  recipients.forEach((userId) => {
    io.to(`user-${userId}`).emit("channel:update", {
      payload: payload,
    });
  });
};
